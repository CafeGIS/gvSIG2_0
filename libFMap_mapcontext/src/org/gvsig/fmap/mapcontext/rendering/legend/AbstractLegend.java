...
public abstract class AbstractLegend implements ILegend{
	 ...
	 */
	private ArrayList listeners = new ArrayList(); //<LegendContentsChangedListener>
	...
	public void fireDefaultSymbolChangedEvent(SymbolLegendEvent event) {
		...
		for (int i = 0; i < listeners.size(); i++) {
			((LegendContentsChangedListener)listeners.get(i)).symbolChanged(event);
		}
	}

	public LegendContentsChangedListener[] getListeners() {
		return (LegendContentsChangedListener[])listeners.toArray(new LegendContentsChangedListener[listeners.size()]);
	}
}
